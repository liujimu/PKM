...
theta = pi/180*30;
isInWorkspace = logical(ones(length(z),length(th),length(r)));
for i = 1:length(z)
    for j = 1:length(th)
        for k = 1:length(r)
            p = [r(k).*cos(th(j)); r(k).*sin(th(j)); z(i)];
            for phi = 0:pi/180*5:2*pi-pi/180*5
                rotm=RotMat(phi,theta,0);
                if ~isinworkspace(p,rotm)
                    isInWorkspace(i,j,k) = false;
                    break
                end
            end
        end
